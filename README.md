...
- **Your AI API key is encrypted at rest** with a non-exportable Android Keystore AES/GCM key and decrypted only in memory at request time ([`KeystoreManager.kt`](app/src/main/java/com/omb9/glucosehero/data/security/KeystoreManager.kt)). Keystore does not guarantee TEE/StrongBox: the app queries `KeyInfo` and logs `insideSecureHardware` for each alias. On emulators and some OEM devices the key is software-backed.
