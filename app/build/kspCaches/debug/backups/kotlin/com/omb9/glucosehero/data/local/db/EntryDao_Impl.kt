...
public class EntryDao_Impl(
  ...
  init {
    ...
    this.__insertAdapterOfEntryEntity = object : EntityInsertAdapter<EntryEntity>() {
      protected override fun createQuery(): String = "INSERT OR ABORT INTO `entries` (`id`,`timestamp`,`glucose_mgdl`,`meal_context`,`insulin_basal_units`,`insulin_bolus_units`,`carbs_grams`,`protein_grams`,`fat_grams`,`meal_description`,`exercise_minutes`,`exercise_intensity`,`note`) VALUES (nullif(?, 0),?,?,?,?,?,?,?,?,?,?,?,?)"
      ...
      protected override fun bind(statement: SQLiteStatement, entity: EntryEntity) {
        ...
        val _tmpProteinGrams: Int? = entity.proteinGrams
        if (_tmpProteinGrams == null) {
          statement.bindNull(8)
        } else {
          statement.bindLong(8, _tmpProteinGrams.toLong())
        }
        val _tmpFatGrams: Int? = entity.fatGrams
        if (_tmpFatGrams == null) {
          statement.bindNull(9)
        } else {
          statement.bindLong(9, _tmpFatGrams.toLong())
        }
        val _tmpMealDescription: String? = entity.mealDescription
        if (_tmpMealDescription == null) {
          statement.bindNull(10)
        } else {
          statement.bindText(10, _tmpMealDescription)
        }
        val _tmpExerciseMinutes: Int? = entity.exerciseMinutes
        if (_tmpExerciseMinutes == null) {
          statement.bindNull(11)
        } else {
          statement.bindLong(11, _tmpExerciseMinutes.toLong())
        }
        val _tmpExerciseIntensity: ActivityIntensity? = entity.exerciseIntensity
        if (_tmpExerciseIntensity == null) {
          statement.bindNull(12)
        } else {
          statement.bindText(12, __ActivityIntensity_enumToString(_tmpExerciseIntensity))
        }
        val _tmpNote: String? = entity.note
        if (_tmpNote == null) {
          statement.bindNull(13)
        } else {
          statement.bindText(13, _tmpNote)
        }
      }
    }
    ...
    this.__updateAdapterOfEntryEntity = object : EntityDeleteOrUpdateAdapter<EntryEntity>() {
      protected override fun createQuery(): String = "UPDATE OR ABORT `entries` SET `id` = ?,`timestamp` = ?,`glucose_mgdl` = ?,`meal_context` = ?,`insulin_basal_units` = ?,`insulin_bolus_units` = ?,`carbs_grams` = ?,`protein_grams` = ?,`fat_grams` = ?,`meal_description` = ?,`exercise_minutes` = ?,`exercise_intensity` = ?,`note` = ? WHERE `id` = ?"
  ...
  public override fun observeEventsSince(since: Long): Flow<List<EntryEntity>> {
    ...
    return createFlow(__db, false, arrayOf("entries")) { _connection ->
      ...
      try {
        ...
        val _columnIndexOfProteinGrams: Int = getColumnIndexOrThrow(_stmt, "protein_grams")
        val _columnIndexOfFatGrams: Int = getColumnIndexOrThrow(_stmt, "fat_grams")
        ...
        while (_stmt.step()) {
          ...
          val _tmpProteinGrams: Int?
          if (_stmt.isNull(_columnIndexOfProteinGrams)) {
            _tmpProteinGrams = null
          } else {
            _tmpProteinGrams = _stmt.getLong(_columnIndexOfProteinGrams).toInt()
          }
          val _tmpFatGrams: Int?
          if (_stmt.isNull(_columnIndexOfFatGrams)) {
            _tmpFatGrams = null
          } else {
            _tmpFatGrams = _stmt.getLong(_columnIndexOfFatGrams).toInt()
          }
          ...
          _item = EntryEntity(_tmpId,_tmpTimestamp,_tmpGlucoseMgdl,_tmpMealContext,_tmpInsulinBasalUnits,_tmpInsulinBolusUnits,_tmpCarbsGrams,_tmpProteinGrams,_tmpFatGrams,_tmpMealDescription,_tmpExerciseMinutes,_tmpExerciseIntensity,_tmpNote)
          _result.add(_item)
        }
        _result
      } finally {
  ...
  public override fun observeGlucosePoints(since: Long): Flow<List<GlucosePointRow>> {
    val _sql: String = """
        |
    ...
    return createFlow(__db, false, arrayOf("entries")) { _connection ->
      ...
      try {
        var _argIndex: Int = 1
        _stmt.bindLong(_argIndex, since)
        val _columnIndexOfTimestamp: Int = 0
        val _columnIndexOfGlucoseMgdl: Int = 1
        val _result: MutableList<GlucosePointRow> = mutableListOf()
        while (_stmt.step()) {
          val _item: GlucosePointRow
          val _tmpTimestamp: Long
          _tmpTimestamp = _stmt.getLong(_columnIndexOfTimestamp)
          val _tmpGlucoseMgdl: Double
          _tmpGlucoseMgdl = _stmt.getDouble(_columnIndexOfGlucoseMgdl)
          _item = GlucosePointRow(_tmpTimestamp,_tmpGlucoseMgdl)
          _result.add(_item)
        }
        _result
      } finally {
    ...
    return createFlow(__db, false, arrayOf("entries")) { _connection ->
      ...
      try {
        ...
        if (_stmt.step()) {
          ...
          _result = EntryEntity(_tmpId,_tmpTimestamp,_tmpGlucoseMgdl,_tmpMealContext,_tmpInsulinBasalUnits,_tmpInsulinBolusUnits,_tmpCarbsGrams,_tmpProteinGrams,_tmpFatGrams,_tmpMealDescription,_tmpExerciseMinutes,_tmpExerciseIntensity,_tmpNote)
        } else {
          _result = null
        }
        _result
  ...
  public override fun observeLoggedDays(since: Long): Flow<List<LoggedDayRow>> {
    val _sql: String = """
        |
    ...
    return createFlow(__db, false, arrayOf("entries")) { _connection ->
      ...
      try {
        ...
        val _result: MutableList<LoggedDayRow> = mutableListOf()
        while (_stmt.step()) {
          val _item: LoggedDayRow
          val _tmpDay: String
          _tmpDay = _stmt.getText(_columnIndexOfDay)
          _item = LoggedDayRow(_tmpDay)
          _result.add(_item)
        }
        _result
      } finally {
  ...
  public override suspend fun loggedDaysSince(since: Long): List<LoggedDayRow> {
    val _sql: String = """
        |
        |        SELECT DISTINCT strftime('%Y-%m-%d', timestamp / 1000, 'unixepoch', 'localtime') AS day
        |        FROM entries
  ...
  public override suspend fun averageGlucoseBetween(startMillis: Long, endMillis: Long): Double? {
    val _sql: String = """
        |
        |        SELECT AVG(glucose_mgdl) FROM entries
        |        WHERE glucose_mgdl IS NOT NULL AND timestamp >= ? AND timestamp < ?
        |        
        """.trimMargin()
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
  ...
  public override suspend fun timeInRangeBetween(
    startMillis: Long,
    endMillis: Long,
    low: Double,
    high: Double,
  ...
  public override suspend fun entriesSince(since: Long): List<EntryEntity> {
    val _sql: String = "SELECT * FROM entries WHERE timestamp >= ? ORDER BY timestamp ASC"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        var _argIndex: Int = 1
  ...
  public override suspend fun latestGlucoseEntry(): EntryEntity? {
    val _sql: String = "SELECT * FROM entries WHERE glucose_mgdl IS NOT NULL ORDER BY timestamp DESC LIMIT 1"
    return performSuspending(__db, true, false) { _connection ->
      val _stmt: SQLiteStatement = _connection.prepare(_sql)
      try {
        val _columnIndexOfId: Int = getColumnIndexOrThrow(_stmt, "id")
