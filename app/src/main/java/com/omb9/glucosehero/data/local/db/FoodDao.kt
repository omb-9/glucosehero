...
interface FoodDao {
    ...
    /**
     * Updates Open Food Facts product data strictly when the row has NOT been corrected
     * by the user (`user_corrected = 0`). Never overwrites a user-corrected row.
     * Returns the number of updated rows (0 if user_corrected was 1 or food not found).
     */
    @Query(
        "UPDATE foods SET " +
            "name = :name, " +
